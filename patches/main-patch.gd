...
const ITEM_SLCT_LOG := "TotoBOPE-ItemSelection:MainPatch"
...
func _spawn_protectors(root: Node):
	...
	ModLoaderLog.debug("Item covers instantiated", ITEM_SLCT_LOG)
...
func _register_death_listener(root: Node):
	...
	ModLoaderLog.debug("Death signal connected", ITEM_SLCT_LOG)
...
func _register_item_steal_listener(root: Node):
	...
	ModLoaderLog.debug("Item steal signal connected", ITEM_SLCT_LOG)
...
func _replace_shell_animations(root: Node):
	...
	if (currentMode != CPGameConfig.GameMode.HIDDEN):
		...
		ModLoaderLog.debug("Shell animations restored", ITEM_SLCT_LOG)
	else:
		...
		ModLoaderLog.debug("Shell animations replaced", ITEM_SLCT_LOG)
	
	
func _on_deathmanager_death():
	ModLoaderLog.debug("Player died. Need to reload main patch!", ITEM_SLCT_LOG)
	applied_main = false
	
func _on_itemmanager_steal_start(root: Node):
	ModLoaderLog.debug("Steal started!", ITEM_SLCT_LOG)
...
func _on_itemmanager_steal_end(root: Node):
	ModLoaderLog.debug("Steal ended!", ITEM_SLCT_LOG)
	var protector = root.get_node("tabletop parent/main tabletop/Protectors")
